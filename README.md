...
Производный от Api. Реализует обмен данными с сервером проекта с помощью хранящихся в объекте класса URL для API и изображений. Содержит методы getItemList для получения с сервера списка товаров, postOrder для отправки заказа, в которые передаётся относительный URL и данные для тела запроса при наличии.
